<?php
/**
 * Plugin Name: Genesis Title Toggle - CentricPro
 * Plugin URI:  http://www.billerickson.net/
 * Description: Turn on/off page titles on a per page basis, and set sitewide defaults from Theme Settings. Must be using the Genesis Centric Pro theme.
 * Author:      Bill Erickson/Neil Gee
 * Author URI:  http://www.billerickson.net/http://wpbeaches.com
 * Version:     1.7.0
...
/**
 * Main BE_NG_Title_Toggle class
 *
 * @since 1.0.0
 * @package BE_NG_Title_Toggle
 */
class BE_NG_Title_Toggle {
	...
	function settings_defaults( $defaults ) {
		...
		$post_types = apply_filters( 'be_ng_title_toggle_post_types', array( 'page' ) );
		foreach ( $post_types as $post_type ) {
			$defaults[] = array( 'be_ng_title_toggle_' . $post_type => '' );
		}
		return $defaults;
	}
	...
	function settings_sanitization() {
		...
		$post_types = apply_filters( 'be_ng_title_toggle_post_types', array( 'page' ) );
		foreach ( $post_types as $post_type ) {
			$fields[] = 'be_ng_title_toggle_' . $post_type;
		}
	...
	function settings_render_metabox() {
		...
		$post_types = apply_filters( 'be_ng_title_toggle_post_types', array( 'page' ) );
		foreach ( $post_types as $post_type ) {
			echo '<p><input type="checkbox" name="' . GENESIS_SETTINGS_FIELD . '[be_ng_title_toggle_' . $post_type . ']" id="' . GENESIS_SETTINGS_FIELD . '[be_ng_title_toggle_' . $post_type . ']" value="1" ' . checked( 1, genesis_get_option( 'be_ng_title_toggle_' . $post_type ), false ) .' /> <label for="' . GENESIS_SETTINGS_FIELD . '[be_ng_title_toggle_' . $post_type . ']"> ' . sprintf( __( 'By default, remove titles in the <strong>%s</strong> post type.', 'genesis-title-toggle' ), $post_type ) .'</label></p>';
		}
	}
	...
	function metabox_render() {
		...
		$default = genesis_get_option( 'be_ng_title_toggle_' . $post_type );

		// Grab current value
		$hide = get_post_meta( get_the_ID(), 'be_ng_title_toggle_hide', true );
		$hide = !empty( $hide ) ? true : false;
		$show = get_post_meta( get_the_ID(), 'be_ng_title_toggle_show', true );
		$show = !empty( $show ) ? true : false;
		
		// Security nonce
		wp_nonce_field( 'be_ng_title_toggle', 'be_ng_title_toggle_nonce' );
		...
		if ( $default ) {
			...
			// Hide by default
			printf( '<label for="be_ng_title_toggle_show">%s</label>', __( 'Show Title', 'genesis-title-toggle' ) );
			
			echo '<input type="checkbox" id="be_ng_title_toggle_show" name="be_ng_title_toggle_show" ' . checked( true , $show, false ) . ' style="margin:0 20px 0 10px;">';
			...
			echo '<input type="hidden" name="be_ng_title_toggle_key" value="show">';

		} else {
		 	
		 	// Show by default
		 	printf( '<label for="be_ng_title_toggle_hide">%s</label>', __( 'Hide Title', 'genesis-title-toggle' ) );
			
			echo '<input type="checkbox" id="be_ng_title_toggle_hide" name="be_ng_title_toggle_hide" ' . checked( true , $hide, false ) . ' style="margin:0 20px 0 10px;">';
		 	...
		 	echo '<input type="hidden" name="be_ng_title_toggle_key" value="hide">';
		}
        ...
        if( get_the_ID() == get_option( 'page_on_front' ) ) { 
            ...
            $h1_site_title = get_post_meta( get_the_ID(), 'be_ng_title_toggle_site_title_h1', true ); 
            $h1_site_title = !empty( $h1_site_title ) ? true : false; 
             
            echo '<p style="padding-top:10px;">'; 
            printf( '<label for="be_ng_title_toggle_site_title_h1">%s</label>', __( 'h1 Site Title', 'genesis-title-toggle' ) ); 
            echo '<input type="checkbox" id="be_ng_title_toggle_site_title_h1" name="be_ng_title_toggle_site_title_h1" ' . checked( true , $h1_site_title, false ) . ' style="margin:0 20px 0 10px;">'; 
	...
	function metabox_save( $post_id, $post ) {
		...
		// Security check
		if ( ! isset( $_POST['be_ng_title_toggle_nonce'] ) || ! wp_verify_nonce( $_POST['be_ng_title_toggle_nonce'], 'be_ng_title_toggle' ) ) {
			return;
		}
		...
		// Which key do we use
		$key = 'be_ng_title_toggle_' . $_POST['be_ng_title_toggle_key'];
        ...
        if( $post_id == get_option( 'page_on_front' ) ) { 
            if( isset( $_POST['be_ng_title_toggle_site_title_h1'] ) ) { 
                update_post_meta( $post_id, 'be_ng_title_toggle_site_title_h1', '1' ); 
            } else { 
                delete_post_meta( $post_id, 'be_ng_title_toggle_site_title_h1' ); 
            }
		}
	}
	...
	function title_toggle() {
		...
		if ( !empty( $default ) ) {
			$override = get_post_meta( $post->ID, 'be_ng_title_toggle_show', true );

			//Centric Pro Specific title removal actions
		...
		// If titles are turned on by default, let's see if this specific one is turned off
			...
			$override = get_post_meta( $post->ID, 'be_ng_title_toggle_hide', true );
    ...
    function site_title_h1( $wrap ) {
            ...
            if( is_front_page() && ! is_home() ) {
                    $show_as_h1 = get_post_meta( get_the_ID(), 'be_ng_title_toggle_site_title_h1', true );
                    if( $show_as_h1 )
                            $wrap = 'h1';
            }
